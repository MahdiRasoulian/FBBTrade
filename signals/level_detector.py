...
class FBBLevelDetector:
    ...
    def detect(self, price:float, bands:dict[str,float], timestamp:datetime, tolerance:float=0.0)->list[FBBLevelEvent]:
        events=[]
        for level in self.levels:
            for side in (LevelSide.UPPER, LevelSide.LOWER):
                key=f'{side.value.lower()}_{level:.3f}'; lp=bands[key]
                active=price>=lp-tolerance if side is LevelSide.UPPER else price<=lp+tolerance
